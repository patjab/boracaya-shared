...
import { daysUntilEventDate, eventDateFromLegacyISO, formatEventDate } from './eventDate.js';
...
export const StringUtils = {
    ...
    isNotEmpty: (str) => !StringUtils.isEmpty(str),
...
export const ArrayUtils = {
    ...
    isNotEmpty: (items) => !ArrayUtils.isEmpty(items),
...
export const NumberUtils = {
    ...
    clamp01: (n) => NumberUtils.clamp(n, 0, 1),
...
export const DateUtils = {
    ...
    calendarDate: (iso, options = { year: 'numeric', month: 'long', day: 'numeric' }, locale = 'en-US') => {
        if (!iso)
            return null;
        const eventDate = eventDateFromLegacyISO(iso);
        return eventDate ? formatEventDate(eventDate, options, locale) : null;
    },
    /** Whole local calendar days to the event date; negative = passed. */
    daysUntil: (dateOrLegacyISO, now = new Date(), eventTimeZone = 'UTC') => {
        const eventDate = eventDateFromLegacyISO(dateOrLegacyISO);
        if (!eventDate)
            return Number.NaN;
        return daysUntilEventDate(eventDate, eventTimeZone, now);
    },
    /**
...
export const ColorUtils = {
    ...
    tintFor: (seed, palette) => palette[ColorUtils.hashCode(seed) % palette.length],
};
