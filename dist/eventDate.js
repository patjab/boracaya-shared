...
exports.daysUntilEventDate = exports.compareEventDates = exports.eventDateInTimeZone = exports.eventCalendarWithLegacyFallback = exports.eventDateFromLegacyISO = exports.formatEventDate = exports.eventDayWindow = exports.supportedEventTimeZones = exports.isEventTimeZone = exports.isEventDate = void 0;
const DATE_PATTERN = /^(\d{4})-(\d{2})-(\d{2})$/;
const isEventDate = (value) => {
...
exports.isEventDate = isEventDate;
...
exports.isEventTimeZone = isEventTimeZone;
const supportedEventTimeZones = () => {
    var _a;
    return [
        'UTC',
...
exports.supportedEventTimeZones = supportedEventTimeZones;
const partsOf = (date) => {
    const match = DATE_PATTERN.exec(date);
    if (!match || !(0, exports.isEventDate)(date))
...
const startOfEventDateMs = (eventDate, eventTimeZone) => {
    if (!(0, exports.isEventTimeZone)(eventTimeZone)) {
...
const eventDayWindow = (eventDate, eventTimeZone) => {
...
exports.eventDayWindow = eventDayWindow;
const formatEventDate = (eventDate, options = {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
}, locale = 'en-US') => {
    if (!(0, exports.isEventDate)(eventDate))
        return null;
    const [year, month, day] = partsOf(eventDate);
...
const eventDateFromLegacyISO = (legacy) => {
    const candidate = legacy === null || legacy === void 0 ? void 0 : legacy.slice(0, 10);
    return (0, exports.isEventDate)(candidate) ? candidate : null;
};
exports.eventDateFromLegacyISO = eventDateFromLegacyISO;
const eventCalendarWithLegacyFallback = (value) => {
    const eventDate = (0, exports.isEventDate)(value.eventDate)
        ? value.eventDate
        : (0, exports.eventDateFromLegacyISO)(value.eventDateISOString);
    if (!eventDate)
        return null;
    return {
        eventDate,
        eventTimeZone: (0, exports.isEventTimeZone)(value.eventTimeZone) ? value.eventTimeZone : 'UTC',
    };
};
exports.eventCalendarWithLegacyFallback = eventCalendarWithLegacyFallback;
const eventDateInTimeZone = (instant, timeZone) => {
    if (!(0, exports.isEventTimeZone)(timeZone))
        throw new RangeError(`Invalid event timezone: ${timeZone}`);
    return dateKeyAt(instant.getTime(), timeZone);
};
exports.eventDateInTimeZone = eventDateInTimeZone;
const compareEventDates = (left, right) => {
    if (!(0, exports.isEventDate)(left) || !(0, exports.isEventDate)(right))
        throw new RangeError('Invalid event date');
    return left === right ? 0 : left < right ? -1 : 1;
};
exports.compareEventDates = compareEventDates;
const daysUntilEventDate = (eventDate, eventTimeZone, now = new Date()) => {
    const today = (0, exports.eventDateInTimeZone)(now, eventTimeZone);
...
exports.daysUntilEventDate = daysUntilEventDate;
